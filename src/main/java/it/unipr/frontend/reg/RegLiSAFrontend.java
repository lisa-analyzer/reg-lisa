 ...
 * Graph (CFG) representation. This class implements the Visitor pattern through
 ...
 * <ul>
 * <li>Each expression node returns a Pair<Statement, Statement> representing
 * entry and exit points</li>
 * <li>Control structures (conditionals, loops) are translated to appropriate
 * branch structures</li>
 * <li>Expressions are translated to their corresponding LiSA statement
 * types</li>
 * </ul>
 * <p>
 *
...
public class RegLiSAFrontend extends RegParserBaseVisitor<Object> {
	...
	/**
	 * Constructs a new RegLiSAFrontend instance to process the specified source
	 * file. Initializes the Program, ClassUnit, CodeMemberDescriptor, and CFG
	 * objects needed to represent the Reg program in LiSA.
	 *
	...
	public RegLiSAFrontend(String file) {
		...
		// Create a descriptor for the "function" that will contain the
		// program's code
		// Since Reg doesn't have functions, we represent the entire program as
		// a single function
		CodeMemberDescriptor cfgDesc = new CodeMemberDescriptor(new SourceCodeLocation(file, 0, 0), unit, false,
				"function");
	...
	/**
	 * Performs the actual work of parsing the input file and constructing the
	 * LiSA Program object. Sets up the ANTLR lexer and parser, then initiates
	 * the AST traversal by visiting the root program node.
	 *
	 * @return The constructed LiSA Program object
	 * 
	 * @throws IOException If an I/O error occurs while reading the file
	...
	/**
	 * Visits the program context node (root of the AST), processes the main
	 * program expression, and completes the CFG construction by adding a return
	 * statement and simplifying the graph.
	 * <p>
	 * This method serves as the entry point for the visitor pattern traversal.
	 * It:
	 * <ol>
	 ...
	 * </ol>
	 * <p>
	 * Grammar rule: <code>program: expr EOF;</code>
	 * <p>
	 * The CFG structure is created as follows:
	 * <p>
	 * <img src="doc-files/visitProgram.jpg" alt="CFG structure for
	 * visitProgram">
	 *
	 * @param ctx The program context from the parser
	 * 
	 * @return The fully constructed LiSA Program object ready for static
	 *             analysis
	 */
	@Override
	...
	public Program visitProgram(RegParser.ProgramContext ctx) {
		...
		SourceCodeLocation loc = new SourceCodeLocation(file, ctx.getStop().getLine(),
	...
	/**
	 * Processes a sequence of expressions by visiting each one and creating
	 * sequential edges between them in the CFG.
	 * <p>
	 ...
	 * <li>The exit point of expression <code>i</code> is connected to the entry
	 * point of expression <code>i + 1</code></li>
	 * <li>The returned pair contains the entry of the first expression and exit
	 * of the last expression</li>
	 * </ul>
	 * <p>
	 ...
	 * </pre>
	 * <p>
	 * Grammar rule: <code>expr: e (SEQ e)*;</code>
	 * <p>
	 * The CFG structure is created as follows:
	 * <p>
	 ...
	 * @return A pair where:
	 *             <ul>
	 *             <li>left: the entry point statement of the first
	 *             expression</li>
	 *             <li>right: the exit point statement of the last
	 *             expression</li>
	 *             </ul>
	 */
	@Override
	...
	public Pair<Statement, Statement> visitSeq(RegParser.SeqContext ctx) {
		...
		for (int i = 1; i < ctx.e().size(); i++) {
			...
			// Connect the exit of the previous expression to the entry of the
			// current one
	...
	/**
	 * Visits a parenthesized expression and delegates to the enclosed
	 * expression.
	 * <p>
	 * Grammar: <code>e: LPAR expr RPAR</code>
	 *
	...
	/**
	 * Visits a no-operation expression (noop) and creates a corresponding NoOp
	 * node in the CFG.
	 * <p>
	 * Grammar: <code>e: NOOP</code>
	 * <p>
	 * The CFG structure is created as follows:
	 * <p>
	...
	public Pair<Statement, Statement> visitNoop(RegParser.NoopContext ctx) {
		...
		// Return the noop as both entry and exit points since it's a single
		// node
		return Pair.of(noop, noop);
	}

	/**
	 * Visits an assignment expression and creates a corresponding Assignment
	 * node in the CFG. If the variable is not declared yet, it adds it to the
	 * descriptor.
	 * <p>
	 * Grammar: <code>e: ID ASSIGN a</code>
	 * <p>
	 * The CFG structure is created as follows:
	 * <p>
	 ...
	 * 
	 * @return A pair containing the Assignment node as both entry and exit
	 *             points
	 */
	@Override
	 ...
	 * <p>
	 * Grammar: <code>e: LPAR e NON_DET_CHOICE e RPAR</code>
	 * <p>
	 * The CFG structure is created as follows:
	 * <p>
	 ...
	 * <ol>
	 * <li><b>Assertion-like condition</b>: When no 'then' branch is provided,
	 * creates a structure where the true branch leads to a NoOp and the false
	 * branch to a return (error case, so an exit point)</li>
	 * <li><b>Standard conditional</b>: Creates a branch where the true path
	 * executes the 'then' statements, and both paths eventually merge at an end
	 * node</li>
	 * </ol>
	 * <p>
	 ...
	 * </pre>
	 * <p>
	 * Grammar rule: <code>e: LPAR b COND (SEQ e (SEQ e)*)? RPAR</code>
	 * <p>
	 * Examples:
	 * <ul>
	 ...
	 * <p>
	 * <img src="doc-files/visitCond - Assert.jpg" alt="CFG structure for
	 * visitCond (assertion-like)">
	 * <li>For standard conditionals:
	 * <p>
	 * <img src="doc-files/visitCond - Classic.jpg" alt="CFG structure for
	 * visitCond (classic)">
	 * </ul>
	 *
	...
	/**
	 * Visits a Kleene star expression (while loop) and creates the
	 * corresponding structure in the CFG with a loop back from the end of the
	 * body to the condition.
	 * <p>
	 * Grammar: <code>e: LPAR b COND SEQ e (SEQ e)* RPAR TIMES</code>
	 * <p>
	 * The CFG structure is created as follows:
	 * <p>
	 ...
	 * 
	 * @return A pair containing the condition node as entry and the end node as
	 *             exit
	 */
	@Override
	...
	/**
	 * Visits an addition or subtraction expression and creates the
	 * corresponding Addition or Subtraction node.
	 * <p>
	 * Grammar: <code>a: a op = (PLUS | MINUS) a</code>
	 *
	...
	/**
	 * Visits a multiplication expression and creates a corresponding
	 * Multiplication node. There's a particular visitor for this operation to
	 * handle precedence correctly.
	 * <p>
	 * Grammar: <code>a: a TIMES a</code>
	 *
	...
	/**
	 * Visits a parenthesized arithmetic expression and delegates to the
	 * enclosed expression.
	 * <p>
	 * Grammar: <code>a: LPAR a RPAR</code>
	 *
	 * @param ctx The parenthesized arithmetic expression context from the
	 *                parser
	 * 
	 * @return The result of visiting the enclosed expression
	 */
	...
	/**
	 * Visits a parenthesized boolean expression and delegates to the enclosed
	 * expression.
	 * <p>
	 * Grammar: <code>b: LPAR b RPAR</code>
	 *
	...
	/**
	 * Visits a comparison expression (equal, less or equal, less than) and
	 * creates the corresponding comparison node.
	 * <p>
	 * Grammar: <code>b: a op = (EQ | LEQ | LE) a</code>
	 *
	 ...
	 *             exit
	 */
	private Pair<Statement, Statement> processConditionalBlock(RegParser.CondContext ctx, Expression cond,
	 ...
	 *             exit
	 */
	private Pair<Statement, Statement> processConditionalBlock(RegParser.KleeneContext ctx, Expression cond,
			SourceCodeLocation loc) {
		return processBlock(ctx.e(), cond, loc, true);
	}

	/**
	 * Creates the CFG structure for a conditional block or loop body based on a
	 * list of expressions.
	 * <p>
	 * This helper method is used by both conditional (if-then) and Kleene star
	 * (loop) constructs to:
	 * <ol>
	 ...
	 * <li>Connect the last statement appropriately based on a construct
	 * type:</li>
	 * <ul>
	 ...
	 * @param isKleene    True if this is a Kleene star (loop), false if it's a
	 *                        conditional
	 * 
	 * @return A pair where:
	 *             <ul>
	 *             <li>left: the condition node (entry point of the
	 *             structure)</li>
	 *             <li>right: the end node (exit point of the structure)</li>
	 *             </ul>
	 */
	@SuppressWarnings("unchecked")
	private Pair<Statement, Statement> processBlock(List<? extends RegParser.EContext> expressions, Expression cond,
			SourceCodeLocation loc, boolean isKleene) {
		...
		// Connect the last statement to either the condition (for loops) or the
		// end node
		if (isKleene) {
		...
		// Connect the false branch of the condition to the end node to create
		// an exit point
